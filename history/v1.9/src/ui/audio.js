...
function ensureYouTubeApiReady(callback) {
  if (state.isYtApiLoaded && window.YT && window.YT.Player) {
    callback();
    return;
  }
      ...
      events: {
        ...
        onStateChange: (ytPlayerEvent) => {
          ...
          if (ytPlayerEvent.data === YT.PlayerState.ENDED) {
            if (state.isMusicShuffleEnabled) {
              playTrack(Math.floor(Math.random() * musicPlaylist.length));
            } else if (state.isMusicLoopEnabled) {
...
function updateMusicPanel() {
  ...
  if (loopBtn) {
    loopBtn.classList.toggle("toggle-on", state.isMusicLoopEnabled);
  }
  if (shuffleBtn) {
    shuffleBtn.classList.toggle("toggle-on", state.isMusicShuffleEnabled);
  }
  if (trackNameDisplay) {
