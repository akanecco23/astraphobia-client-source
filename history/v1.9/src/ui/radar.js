...
function drawRadar(ctx, canvas, gameData) {
  ...
  if (window.entityTrailEnabled && window.entityTrailTargetId) {
    ...
    if (targetEntityId) {
      ...
      if (targetEntityPos) {
        const deltaX = targetEntityPos.x - gameData.myPos.x;
        const deltaY = targetEntityPos.y - gameData.myPos.y;
        const renderX = Math.max(
          radarX + 2,
          Math.min(
            radarX + radarSize - 2,
            radarX + radarSize / 2 + deltaX * pixelPerUnit,
          ),
        );
        const renderY = Math.max(
          ...
          Math.min(
            radarY + radarSize - 2,
            radarY + radarSize / 2 + deltaY * pixelPerUnit,
          ),
        );
        ...
        ctx.strokeStyle = "rgba(" + rgbValue + "," + opacity + ")";
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(renderX, renderY, 7, 0, Math.PI * 2);
        ctx.stroke();
        ctx.strokeStyle = "rgba(" + rgbValue + "," + opacity * 0.5 + ")";
        ctx.lineWidth = 4;
        ctx.beginPath();
