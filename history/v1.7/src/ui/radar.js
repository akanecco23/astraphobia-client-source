...
function drawRadar(ctx, canvas, gameState) {
  ...
  if (window.entityTrailEnabled && window.entityTrailTargetId) {
    ...
    if (targetEntityId) {
      ...
      if (targetEntity) {
        const deltaY = targetEntity.x - gameState.myPos.x;
        const deltaY_xp7 = targetEntity.y - gameState.myPos.y;
        const drawY = Math.max(
          radarX + 2,
          Math.min(
            radarX + radarSize - 2,
            radarX + radarSize / 2 + deltaY * radarScale,
          ),
        );
        const drawY_xpg = Math.max(
          radarY + 2,
          Math.min(
            radarY + radarSize - 2,
            radarY + radarSize / 2 + deltaY_xp7 * radarScale,
          ),
        );
        const alpha = Math.sin(Date.now() / 200) * 0.3 + 0.7;
        const { r: red, g: blue, b: blue_y68 } = window.entityTrailColor;
        const rgbValue = red + "," + blue + "," + blue_y68;
        ctx.strokeStyle = "rgba(" + rgbValue + "," + alpha + ")";
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(drawY, drawY_xpg, 7, 0, Math.PI * 2);
        ctx.stroke();
        ctx.strokeStyle = "rgba(" + rgbValue + "," + alpha * 0.5 + ")";
        ctx.lineWidth = 4;
        ctx.beginPath();
        ctx.arc(drawY, drawY_xpg, 10, 0, Math.PI * 2);
        ctx.stroke();
        ctx.fillStyle = "rgb(" + rgbValue + ")";
        ctx.beginPath();
        ctx.arc(drawY, drawY_xpg, 3, 0, Math.PI * 2);
        ctx.fill();
